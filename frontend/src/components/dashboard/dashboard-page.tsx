...
const DashboardPage: React.FC<DashboardPageProps> = ({ className = "" }) => {
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {!orgId && !orgLoading && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-600">
            No organization ID provided. Please access this page with a valid
            organization ID.
          </p>
        </div>
      )}
      ...
      {orgError && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-600">Error loading organization: {orgError}</p>
        </div>
      )}
      ...
      {orgId && !orgLoading && !orgError && !agentsLoading && !agentsError && (
